...
function turnintosN(number: string) {
    
    let copy = number.split("")
    let power
    ...
    if(copy[0] === "0"){
        ...
        for(let i = 0; i < copy.length; i++){
            if(copy[i] !== "0" && copy[i] !== "."){
                ...
                const newc = copy.slice(power)

                newc.splice(1, 0, ".")

                console.log(newc.join("") + "10^-" + power)
    }   

    else{
        const copy = number.split("")

        if(!copy.includes(".")){
            const powerone = copy.length - 1
            copy.splice(1, 0, ".")

            console.log(copy.join("")+ "*"+"10^"+powerone)
            return
        }

        const indexofp = copy.indexOf(".")
        copy.splice(indexofp, 1)

        const powertwo = indexofp - 1

        copy.splice(indexofp - powertwo, 0, ".")

        console.log(copy.join("") + "10^" + powertwo)
    }
}
...
function findifnorepeat(num: number): void {

    let numm: number

    if(num === 1){
        console.log("number is not repeating")
        return
    }

    if(num % 2 === 0){
        numm = num/2
        return findifnorepeat(numm)
    }
    ...
    console.log("number is repeating")
